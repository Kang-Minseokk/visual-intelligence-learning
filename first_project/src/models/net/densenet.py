from typing import Iterable

from torch import nn

from src.models.linear.densenet_linear import DenseBlock, TransitionLayer
from src.models.net.model_base import ModelUtilMixin


class DenseNet(ModelUtilMixin):
    def __init__(
        self,
        in_channels: int,
        num_classes: int = 10,
        growth_rate: int = 12,
        block_config: Iterable[int] = (6, 12, 24, 16),
        num_init_features: int = 24,
        bn_size: int = 4,
        drop_rate: float = 0.0,
        compression: float = 0.5,
    ):
        super().__init__()

        if not (0 < compression <= 1.0):
            raise ValueError(f"compression must be in (0, 1], got: {compression}")

        block_config = tuple(int(v) for v in block_config)
        if len(block_config) == 0:
            raise ValueError("block_config must contain at least one block")

        self.flatten = nn.Identity()

        features = [
            nn.Conv2d(in_channels, num_init_features, kernel_size=3, stride=1, padding=1, bias=False)
        ]
        num_features = num_init_features

        for i, num_layers in enumerate(block_config):
            block = DenseBlock(
                num_layers=num_layers,
                in_channels=num_features,
                growth_rate=growth_rate,
                bn_size=bn_size,
                drop_rate=drop_rate,
            )
            features.append(block)
            num_features = num_features + num_layers * growth_rate

            if i != len(block_config) - 1:
                out_features = int(num_features * compression)
                features.append(TransitionLayer(in_channels=num_features, out_channels=out_features))
                num_features = out_features

        features.append(nn.BatchNorm2d(num_features))
        features.append(nn.ReLU(inplace=True))
        self.feature_extractor = nn.Sequential(*features)

        self.classifier = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),
            nn.Flatten(),
            nn.Linear(num_features, num_classes),
        )
